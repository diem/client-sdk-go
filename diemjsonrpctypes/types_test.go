	...
	cases := []struct {
	...
	}{
		{
			...
			unmarshal: func(t *testing.T, resp *jsonrpc.Response) interface{} {
				var result []*diemjsonrpctypes.Transaction
		...
		},
		{
			...
			unmarshal: func(t *testing.T, resp *jsonrpc.Response) interface{} {
				var result diemjsonrpctypes.Account
				ok, err := resp.UnmarshalResult(&result)
				assert.True(t, ok)
				require.NoError(t, err)
				return &result
		...
		},
		{
			...
			unmarshal: func(t *testing.T, resp *jsonrpc.Response) interface{} {
				var result []*diemjsonrpctypes.CurrencyInfo
				ok, err := resp.UnmarshalResult(&result)
				assert.True(t, ok)
				require.NoError(t, err)
				return &result
		...
		},
		{
			...
			unmarshal: func(t *testing.T, resp *jsonrpc.Response) interface{} {
				var result diemjsonrpctypes.Metadata
				ok, err := resp.UnmarshalResult(&result)
				assert.True(t, ok)
				require.NoError(t, err)
				return &result
		...
		},
		{
			...
			unmarshal: func(t *testing.T, resp *jsonrpc.Response) interface{} {
				var result []*diemjsonrpctypes.Event
				ok, err := resp.UnmarshalResult(&result)
				assert.True(t, ok)
				require.NoError(t, err)
				return &result
